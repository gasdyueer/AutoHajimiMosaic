...
def tile_pattern(image_size, pattern_image):
    ...
    tiled_pattern = np.tile(
        pattern_image,
        (image_size[1] // pattern_height + 1, image_size[0] // pattern_width + 1, 1),
    )
...
def apply_mask(
    ...
    if mask_indices.size > 0:
        ...
        transformed_head = cv2.warpPerspective(
            head_image, matrix, image.shape[:2][::-1]
        )

        alpha_channel = (transformed_head[:, :, 3] / 255)[:, :, np.newaxis]
        image_with_head = (
            alpha_channel * transformed_head[:, :, :3] + (1 - alpha_channel) * image
        ).astype(np.uint8)
