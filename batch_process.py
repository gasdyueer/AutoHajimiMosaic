...
def process_images(
    input_folder, output_folder, pattern_image_path, head_image_path, recursive
):
        ...
        if file.is_file() and file.suffix[1:].lower() in support_ext:
            ...
            try:
                ...
                if category_name in ["porn", "hentai"]:
                    print(
                        f"Warning: {file} is classified as sensitive content, but no mask was found."
                    )
                masks = []
                class_ids = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Batch process images with YOLO models."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to the head image.",
    )
    parser.add_argument(
        "--recursive", action="store_true", help="Include subfolders in processing."
    )
